...
angular.module('snippit.three', ['snippit'])
  .controller('ThreeController', ['$scope', 'ThreeFactory', '$window', '$document', 'Facebook', function($scope, ThreeFactory, $window, $document, Facebook) {
    ...
    var init = function(){
      ...
      var len = picData.thumbnails.length

      for (var i = 0; i < len; i++) {
        ThreeFactory.createObject(i, picData.thumbnails, scene, $scope.objects, $scope.hit);
    ...
    $scope.hit = function(){
      Modal.open({
         content: "<div class='imageResize'><img src='"+picData.pictures[this]+"' /></div>",
         draggable: false,
         width: 'auto',
         context: this
         // hideClose: true
    ...
    $scope.getPics = function(){
      ...
      Facebook.getWallData()
        .then(function(resp){
          ...
          console.log(data)
          picData = {pictures: data.wallPhotos.picture,
                     thumbnails: data.wallPhotos.thumbnail
                    }
          var picEls = document.getElementsByClassName('picImg');
          for (var i = 0; i < picEls.length; i++) {
            picEls[i].src = picData.thumbnails[i];
          };
          console.log(picEls);
